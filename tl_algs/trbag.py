...
def aucec50_metric(clf, X_test, y_test):
    '''Metric for aucec50'''
    return vuln_metrics.aucec_50_score(y_test.tolist(), [a[1] for a in clf.predict_proba(X_test)])
...
def count_vote(F_star, test_set_X, threshold_prop=0.5):
    ...
    confidence_arr = [float(sum(instance_votes)) / len(F_star) for instance_votes in votes ]
    # prediction = confidence > threshold_count
    prediction_arr = [confidence > threshold_count for confidence in confidence_arr]
...
def mean_confidence_vote(F_star, test_set_X):
    ...
    for f in F_star:
        ...
        # give the confidence for the positive class (vulnerable), unless there is no confidence
    ...
    # return the mean confidence and if the confidence is greater than .5, predict vulnerability
...
def sc_trbag_filter(f_0, F, X_target, y_target, metric=aucec50_metric):
    ...
    for f in F:
        #append f if it has a better performance on the target set than the fallback f_0
...
def mvv_filter(f_0, F, X_target, y_target, raw_metric=aucec50_raw_metric, vote_func=mean_confidence_vote):
    '''
    '''
    fallback_predicted = f_0.predict(X_target)
    proba = f_0.predict_proba(X_target)
    # give the confidence for the positive class (vulnerable), unless there is no confidence
    fallback_confidence = [a[1] for a in proba] if len(proba[0]) > 1 else [0 for a in proba]
    fallback_performance = raw_metric(y_target, list(fallback_predicted), list(fallback_confidence))
    ...
    F_scores = [raw_metric(y_target, f.predict(X_target), [a[1] for a in proba] if len(proba[0]) > 1 else [0 for a in proba]) for f in F]
    #print(F_scores)
    ...
    for f in F_sorted:
        ...
        # only select candidate set if its performance is better than the fallback classifier alone
...
class TrBag(tl_alg.Base_Transfer):
    ...
    def __init__(self, test_set_X, test_set_proj, train_pool_X, 
	   ...
	   # this should be passing all the base parameters to the superclass
        super(TrBag, self).__init__(test_set_X, test_set_proj, train_pool_X, 
			 train_pool_y, train_pool_proj, 
			 Base_Classifier, rand_seed=rand_seed, classifier_params=classifier_params)
...
# ----------------Driver----------------
    ...
    def split_validation(self, test_set_proj, train_pool_X, train_pool_y, train_pool_proj, validate_proportion, rand_seed):
	   ...
	   if (validate_proportion is None):
		  ...
		  X_validate = X_target.sample(frac=validate_proportion, random_state=rand_seed)
		  ...
		  # we are able to use iloc (position based indexing) because we reset the index above
		  X_target_train = X_target.ix[X_target.index.difference(X_validate.index),]
		  ...
		  train_pool_X = train_pool_X.ix[train_pool_X.index.difference(X_validate.index),]
		  train_pool_y = train_pool_y[train_pool_X.index]
	   return (X_target_train, y_target_train, X_validate, y_validate, train_pool_X, train_pool_y)

    def bootstrap(self, train_pool_X, train_pool_y, Base_Classifier, T, bag_prop, rand_seed, classifier_params):
	   ...
	   for i in range(0,T):
		  ...
		  X_bootstrap = train_pool_X.sample(bag_prop * len(train_pool_X), replace=True, random_state=rand_seed + i)
    ...
    def trbag_validate(self, test_set_X, test_set_proj, train_pool_X, train_pool_y, train_pool_proj, Base_Classifier, classifier_params={}, T=100, bag_prop=1.0,
		  filter_func=sc_trbag_filter, vote_func=count_vote, validate_proportion=None, rand_seed=None):
	   ''' Apply the trBag method of transfer learning and return a tuple of the form (confidence_array, predicted_classes)
	   T: number of classifiers to bootstrap on (plus one baseline classifier)
	   ...
	   (X_target_train, y_target_train, X_validate, y_validate, train_pool_X, train_pool_y) = self.split_validation(test_set_proj,
		  train_pool_X, train_pool_y, train_pool_proj, validate_proportion, rand_seed)

	   # bootstrap sample to generate T learners
	   F = self.bootstrap(train_pool_X, train_pool_y, Base_Classifier, T, bag_prop, rand_seed, classifier_params)
	   ...
	   if (X_target_train.shape[0] == 0):
		  ...
		  f_0 = DummyClassifier(strategy='uniform', random_state=rand_seed).fit(train_pool_X, train_pool_y.tolist())
	   else :
		  f_0 = Base_Classifier(random_state=rand_seed, **classifier_params).fit(X_target_train, y_target_train.tolist())
    ...
    def train_filter_test(self):
        return self.trbag_validate(self.test_set_X, self.test_set_proj, self.train_pool_X, self.train_pool_y, self.train_pool_proj, 
                self.Base_Classifier, classifier_params=self.classifier_params, T=self.T, bag_prop=self.bag_prop,
    		  filter_func=self.filter_func, vote_func=self.vote_func, validate_proportion=self.validate_proportion, rand_seed=self.rand_seed)
    
    def json_encode(self):
        base = tl_alg.Base_Transfer.json_encode(self)
        base.update({"T": self.T, "bag-prop" : self.bag_prop, "filter_func" : self.filter_func.__name__, "vote_func" : self.vote_func.__name__, "validate_prop" : self.validate_proportion})
        return base
