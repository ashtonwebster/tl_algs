...
class GravityWeight(tl_alg.Base_Transfer):
    ...
    def __init__(self, test_set_X, test_set_proj, train_pool_X, 
			 ...
			 Base_Classifier, rand_seed=None, classifier_params={}, similarity_func=sim_std):
        ...
        super(GravityWeight, self).__init__(test_set_X, test_set_proj, train_pool_X, 
			 train_pool_y, train_pool_proj, 
    ...
    def train_filter_test(self):
        ...
        def isinrange(row):
                return sum([cell >= ranges[i][0] and cell <= ranges[i][1] for i, cell in enumerate(row)])
        #for each row, compute similarity
        similarity_count = self.train_pool_X.apply(isinrange, axis=1)
        weight_vec = map(lambda x: float(float(x) / (len(ranges) - x + 1)**2), similarity_count)
        # apply classification with sample weight.
        f = self.Base_Classifier(random_state=self.rand_seed, 
                **self.classifier_params).fit(self.train_pool_X, list(self.train_pool_y), 
                sample_weight=np.array(weight_vec))
        confidence = f.predict_proba(self.test_set_X)

        return  ([a[-1] for a in confidence] if len(confidence[0]) > 1 else list(confidence), f.predict(self.test_set_X))
